...
export default function Project() {
  ...
  // edit panel
  const [editProject, setEditProject] = useState(null)  // project object
  ...
  async function saveEdit() {
    ...
    const { data } = await supabase
      ...
      .order('created_at')
    setItems(prev => ({ ...prev, [editProject.id]: data ?? [] }))

    // refresh project name in list
  ...
  async function removeEditItem(itemId) {
    ...
    setItems(prev => ({
      ...prev,
      [editProject.id]: (prev[editProject.id] ?? []).filter(i => i.id !== itemId)
    }))
  }

  function setEditQty(itemId, val) {
    setEditItems(prev => prev.map(i => i.id === itemId ? { ...i, quantity: val } : i))
  }

  function setEditUnit(itemId, val) {
    setEditItems(prev => prev.map(i => i.id === itemId ? { ...i, unit: val } : i))
  }
          ...
          {editProject && (
            <div className="w-80 flex-shrink-0 card">
              <div className="flex items-center justify-between mb-5">
                ...
                <button onClick={() => setEditProject(null)} className="text-muted hover:text-cream transition-colors">
                  <X size={16} />
                </button>
              </div>
                      ...
                      <div key={item.id} className="border border-forest-light/30 rounded-lg p-2.5">
                        ...
                        <div className="flex gap-2">
                          <input
                            ...
                            className="flex-1 bg-forest border border-forest-light/40 rounded-lg px-2 py-1
                                       text-cream text-xs font-mono focus:outline-none focus:border-gold/50"
                          />
                          <select
                            value={item.unit}
                            onChange={e => setEditUnit(item.id, e.target.value)}
                            className="bg-forest border border-forest-light/40 rounded-lg px-2 py-1
                                       text-cream text-xs font-mono focus:outline-none focus:border-gold/50
                                       appearance-none cursor-pointer w-14"
                          >
